...
% 2) The goal is to create 6 samples (lists) of 200 words matched on
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
clear all
worddir = '/rri_disks/artemis/meltzer_lab/NIBS_treatment/wordrep_paradigm/stim_creation/';
addpath(genpath([worddir 'SOS']));
cd([worddir '2017-02-09_stim_selection'])
...
if strcmp(includeverbs, 'yes')
    wordrep_Population = population('Master_wordlist_sostext_updatedzipcat.txt',...
        'name', 'wordrep_Population', 'isHeader',true, 'isFormatting', true);
else
    wordrep_Population = population('2017-02-27_subpopulation_allvars.txt', ...
        'name', 'wordrep_Population', 'isHeader',true, 'isFormatting', true);
end
...
% Specifies the type of optimization (default: 'greedy')
    ...
    %expAnneal.maxpDecrease(GreedySOS.deltaCost, 0.00034228,10)
    
    case 'EXP'
   
    StochasticSOS = GreedySOS;
    StochasticSOS.setAnnealSchedule('schedule', 'exp', 'pDecrease', 0.470294);
