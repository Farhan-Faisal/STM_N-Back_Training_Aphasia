...
%      CREATESOFTENTROPYCONSTRAINT_DIALOG, by itself, creates a new CREATESOFTENTROPYCONSTRAINT_DIALOG or raises the existing
%      singleton*.
%
%      H = CREATESOFTENTROPYCONSTRAINT_DIALOG returns the handle to a new CREATESOFTENTROPYCONSTRAINT_DIALOG or the handle to
%      the existing singleton*.
%
%      CREATESOFTENTROPYCONSTRAINT_DIALOG('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in CREATESOFTENTROPYCONSTRAINT_DIALOG.M with the given input arguments.
%
%      CREATESOFTENTROPYCONSTRAINT_DIALOG('Property','Value',...) creates a new CREATESOFTENTROPYCONSTRAINT_DIALOG or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before createSoftEntropyConstraint_Dialog_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to createSoftEntropyConstraint_Dialog_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help createSoftEntropyConstraint_Dialog

% Last Modified by GUIDE v2.5 17-Sep-2010 13:18:27

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @createSoftEntropyConstraint_Dialog_OpeningFcn, ...
                   'gui_OutputFcn',  @createSoftEntropyConstraint_Dialog_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before createSoftEntropyConstraint_Dialog is made visible.
function createSoftEntropyConstraint_Dialog_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to createSoftEntropyConstraint_Dialog (see VARARGIN)

% Choose default command line output for createSoftEntropyConstraint_Dialog
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes createSoftEntropyConstraint_Dialog wait for user response (see UIRESUME)
% uiwait(handles.figure_createSoftEntropyConstraint_Dialog);


% --- Outputs from this function are returned to the command line.
function varargout = createSoftEntropyConstraint_Dialog_OutputFcn(hObject, eventdata, handles) 
                        ...
                        if create == true
                            ...
                            if create == true
                                exp = get (handles.edit_exp,'String');
                                    ...
                                    if isempty(nbin)
                                        % run abbreviated command if no bin
                                    ...
                                    verbosePrint(['Executing command: ','''',command,''''],'createSoftEntropyConstraint_Dialog_create');
                                    evalin('base',command);      
                                    close(handles.figure_createSoftEntropyConstraint_Dialog);   
                            end
                        end
                   end
                end
