% - create two sample soft distance constraint dialog
%
...
function varargout = createTwoSampleSoftDistanceConstraint_Dialog(varargin)
% CREATETWOSAMPLESOFTDISTANCECONSTRAINT_DIALOG M-file for createTwoSampleSoftDistanceConstraint_Dialog.fig
%      CREATETWOSAMPLESOFTDISTANCECONSTRAINT_DIALOG, by itself, creates a new CREATETWOSAMPLESOFTDISTANCECONSTRAINT_DIALOG or raises the existing
%      singleton*.
%
%      H = CREATETWOSAMPLESOFTDISTANCECONSTRAINT_DIALOG returns the handle to a new CREATETWOSAMPLESOFTDISTANCECONSTRAINT_DIALOG or the handle to
%      the existing singleton*.
%
%      CREATETWOSAMPLESOFTDISTANCECONSTRAINT_DIALOG('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in CREATETWOSAMPLESOFTDISTANCECONSTRAINT_DIALOG.M with the given input arguments.
%
%      CREATETWOSAMPLESOFTDISTANCECONSTRAINT_DIALOG('Property','Value',...) creates a new CREATETWOSAMPLESOFTDISTANCECONSTRAINT_DIALOG or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before createTwoSampleSoftDistanceConstraint_Dialog_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to createTwoSampleSoftDistanceConstraint_Dialog_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help createTwoSampleSoftDistanceConstraint_Dialog

% Last Modified by GUIDE v2.5 16-Sep-2010 20:58:22

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @createTwoSampleSoftDistanceConstraint_Dialog_OpeningFcn, ...
                   'gui_OutputFcn',  @createTwoSampleSoftDistanceConstraint_Dialog_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before createTwoSampleSoftDistanceConstraint_Dialog is made visible.
function createTwoSampleSoftDistanceConstraint_Dialog_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to createTwoSampleSoftDistanceConstraint_Dialog (see VARARGIN)

% Choose default command line output for createTwoSampleSoftDistanceConstraint_Dialog
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes createTwoSampleSoftDistanceConstraint_Dialog wait for user response (see UIRESUME)
% uiwait(handles.figure_createTwoSampleSoftDistanceConstraint_Dialog);


% --- Outputs from this function are returned to the command line.
function varargout = createTwoSampleSoftDistanceConstraint_Dialog_OutputFcn(hObject, eventdata, handles) 
...
% --- Executes on button press in pushbutton_createTwoSampleSoftDistanceConstraint.
function pushbutton_createTwoSampleSoftDistanceConstraint_Callback(hObject, eventdata, handles)
% hObject    handle to pushbutton_createTwoSampleSoftDistanceConstraint (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
                                ...
                                if create == true
                                    exp = get (handles.edit_exp,'String');
                                        ...
                                        fnc = getdfName(handles.popupmenu_fncSelector,...
                                                        'fnc');
                                        stat = getdfName(handles.popupmenu_statSelector,...
                                                        'stat');
                                        paired = getdfName(handles.popupmenu_pairedSelector,...
                                                'paired');
                                        ...
                                        verbosePrint(['Executing command: ','''',command,''''],'createTwoSampleSoftDistanceConstraint_Dialog_create');
                                        evalin('base',command);      
                                        close(handles.figure_createTwoSampleSoftDistanceConstraint_Dialog);   
                                end
                            end
                       end
                   end
