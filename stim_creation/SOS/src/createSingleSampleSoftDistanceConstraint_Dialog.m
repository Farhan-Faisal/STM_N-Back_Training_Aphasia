% - create single sample soft distance constraint dialog
%
...
function varargout = createSingleSampleSoftDistanceConstraint_Dialog(varargin)
% CREATESINGLESAMPLESOFTDISTANCECONSTRAINT_DIALOG M-file for createSingleSampleSoftDistanceConstraint_Dialog.fig
%      CREATESINGLESAMPLESOFTDISTANCECONSTRAINT_DIALOG, by itself, creates a new CREATESINGLESAMPLESOFTDISTANCECONSTRAINT_DIALOG or raises the existing
%      singleton*.
%
%      H = CREATESINGLESAMPLESOFTDISTANCECONSTRAINT_DIALOG returns the handle to a new CREATESINGLESAMPLESOFTDISTANCECONSTRAINT_DIALOG or the handle to
%      the existing singleton*.
%
%      CREATESINGLESAMPLESOFTDISTANCECONSTRAINT_DIALOG('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in CREATESINGLESAMPLESOFTDISTANCECONSTRAINT_DIALOG.M with the given input arguments.
%
%      CREATESINGLESAMPLESOFTDISTANCECONSTRAINT_DIALOG('Property','Value',...) creates a new CREATESINGLESAMPLESOFTDISTANCECONSTRAINT_DIALOG or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before createSingleSampleSoftDistanceConstraint_Dialog_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to createSingleSampleSoftDistanceConstraint_Dialog_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help createSingleSampleSoftDistanceConstraint_Dialog

% Last Modified by GUIDE v2.5 16-Sep-2010 17:16:23

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @createSingleSampleSoftDistanceConstraint_Dialog_OpeningFcn, ...
                   'gui_OutputFcn',  @createSingleSampleSoftDistanceConstraint_Dialog_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before createSingleSampleSoftDistanceConstraint_Dialog is made visible.
function createSingleSampleSoftDistanceConstraint_Dialog_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to createSingleSampleSoftDistanceConstraint_Dialog (see VARARGIN)

% Choose default command line output for createSingleSampleSoftDistanceConstraint_Dialog
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes createSingleSampleSoftDistanceConstraint_Dialog wait for user response (see UIRESUME)
% uiwait(handles.figure_createSingleSampleSoftDistanceConstraint_Dialog);


% --- Outputs from this function are returned to the command line.
function varargout = createSingleSampleSoftDistanceConstraint_Dialog_OutputFcn(hObject, eventdata, handles) 
                        ...
                        if create == true
                            ...
                            if create == true
                                exp = get (handles.edit_exp,'String');
                                    ...
                                    verbosePrint(['Executing command: ','''',command,''''],'createSingleSampleSoftDistanceConstraint_Dialog_create');
                                    evalin('base',command);      
                                    close(handles.figure_createSingleSampleSoftDistanceConstraint_Dialog);   
                            end
                        end
                   end
                end
